...
def follow(request, u_id):
    ...
    if f_user.exists():
        follow = UserFollowing.objects.create(user_id=request.user, following_user_id=f_user.first())
...
def unFollow(request, u_id):
    ...
    if f_user.exists():
        try:
            UserFollowing.objects.get(user_id=request.user.id, following_user_id=u_id).delete()
